...
| `py-marshal-loader/` | **Remote deserialize-and-execute dropper** (OSSF/PyPI archetype): import-time `exec(marshal.loads(<remote>))` runs a marshalled code object fetched from a C2 — the serializer variant of decode-and-execute. | `ST-OBF-DECODE-EXEC-PY` (malicious indicator, HIGH), `ST-NET-PY`, `ST-DYN-PY` → **malicious verdict** (aggregate risk LOW per the single-HIGH note above; CI gates on the HIGH finding) |
| `sh-base64-exec/` | **Shell bootstrap dropper**: an `install.sh` that decodes and executes an obfuscated first stage (`… base64 -d \| bash`) and pipes a remote second stage straight into a shell (`curl … \| bash`). | `ST-OBF-DECODE-EXEC-SH` (malicious indicator, HIGH), `ST-SHELL-PIPE-EXEC` (risky, HIGH) → **malicious verdict** |
